...
class Multimode: # Définition de la classe OD
    ...
    def tptransit(self, timestamp, s_id, s_olat, s_olng, s_dlat, s_dlng, keytptransit):
        ...
        self.elementliste = self.Element[0]['status']
        ...
        try:
            self.statu == 'OK'
            try:
                self.elementliste = 'ZERO_RESULTS'
                ...
                return (self.s_id,self.mdureemin)
            except:
                self.mdureemin = 0
                return (self.s_id,self.mdureemin)
    ...
    def train(self, timenavitia, distmax, s_id, s_olat, s_olng, s_dlat, s_dlng, token):
        ...
        self.urlaaaa = '&datetime_represents=arrival&datetime='
        self.urlfiltre = '&first_section_mode[]=walking&first_section_mode[]=bike&first_section_mode[]=bss&allowed_id[]=physical_mode:TrainTER&forbidden_uris[]=physical_mode:Bus'
        self.urlb = '&to='
        ...
        self.urlq = self.urla+self.urlaa+self.coverage+self.urlaaa+self.s_norigine+self.urlb+self.s_ndestination+self.urlc+self.urlcc+self.urlaaaa+self.timenavitia+self.urlfiltre
        #&allowed_id[]=physical_mode:TrainTER
        #forbidden_uris[]=physical_mode:TrainTER
        print (self.urlq)
        self.resp = requests.get(self.urlq, headers={'Authorization':self.token})
        try:
            self.nav_output = self.resp.json()
            #pprint(self.nav_output)
            ...
            if self.lena >1:
                ...
                for i in range(len(self.journey_list)):
                    self.sections_list = self.journey_list[0]['sections']
                    ...
                    for section in self.journey_list[0]['sections']:
                        
                        
                        
                        try:
                            self.sections_list[0]['mode']
                            pprint(section['mode'])
                            
                            self.sectionmode = self.sections_list[0]['mode']
                            self.secres.append(self.sectionmode)
                        except:
                            self.error = "pas de mode"
                            return '','',self.error,''

                            
                    i=+1
                
                return (self.nduration,self.corres,self.type, self.secres)
    ...
    def htrain(self, timenavitia, distmax, s_id, s_olat, s_olng, s_dlat, s_dlng, token):
        """Fonction de recherche du premier temps TC
        théorique fournis par l'API Navitia.io"""
        self.token = token       
        self.s_id = s_id
        ...
        self.urlaaa = '/journeys?from='
        self.urlaaaa = '&datetime_represents=arrival&datetime='
        self.urlfiltre = '&first_section_mode[]=walking&first_section_mode[]=bike&forbidden_uris[]=physical_mode:LocalTrain'
        self.urlb = '&to='
        self.urlc = '&max_duration_to_pt='
        self.urlcc = str(self.distmmax * 60)
        #selfurlq = 'https://api.navitia.io/v1/coverage/'+self.coverage+'/journeys?from='+self.s_norigine+'&to='+self.s_ndestination+self.timenavitia+'&max_duration_to_pt=900'
        ...
        try:
            ...
            if self.lena >1:
                ...
                for i in range(len(self.journey_list)):
                    ...
                    self.corres = self.journey_list[0]['nb_transfers']
                    self.type = self.journey_list[0]['type'] #ne donne que le premier résultat - TODO changer [0] en [i] et le return pour avoir une liste
                    self.section1 = self.sections_list[1]['display_informations']['physical_mode']
                    i=+1
                
                return (self.nduration,self.corres,self.type,self.section1)
            else:
                self.error = [self.nav_output['error']['message']]
                return '','',self.error,''
        ...
        with open(self.fichiersal) as f:
            ...
            with open(self.resultfichier, 'w')as sortie:
                ...
                for row in reader:
                    ...
                    #idsal  = sal.tpvp(s_id,s_olat,s_olng,s_dlat,s_dlng, keytpvp)
                    ...
                    nduration, ncorres, ntype, section1 = sal.train(self.timenavitia, self.distmax, s_id,s_olat,s_olng,s_dlat,s_dlng,token)
                    htnduration, htncorres, htntype, htsection1 = sal.htrain(self.timenavitia, self.distmax, s_id,s_olat,s_olng,s_dlat,s_dlng,token)
                    idsal, tptransit = sal.tptransit(self.timestamp, s_id,s_olat,s_olng,s_dlat,s_dlng,keytptransit)

                    #result = idsal,s_olat,s_olng,s_dlat,s_dlng,distkm,tpvp,tptrafic,tpmarche,gvtime,nduration,ncorres,ntype
                    result = idsal, tptransit, nduration, ncorres, ntype
                    ...
                    self.resultgui = ("Trajet " + resultmessage_idtra + " sur " + (str(self.line_resultv)) +  ". Temps Navitia : " + resultmessage_navitia)
                    ...
                    #self.resultgui = str(self.resultgui).replace(" ",'')
                    #self.resultgui = str(self.resultgui).replace(",",';')
                    geom = 'Point ('+str(s_olng)+' '+str(s_olat)+')'

                    #print(self.resultgui)
                    csv_out.writerow((idsal,geom,nduration,ncorres,ntype,section1, htnduration,htncorres,htntype,htsection1,tptransit))
                    self.line_result = str(self.lines)
